...
// Assign the values from the request to variables
...
if (!empty($errors)) {
    ...
    header('Content-Type: application/json; charset=utf-8');
    die(json_encode(array("message" => "User creation failed", "errors" => $errors)));
}
...
if($statement->execute()) {
    ...
    die(json_encode(array("message" => "User create successfully")));
} else {
    http_response_code(400);
    die(json_encode(array("message" => "Failed to create user")));
}
